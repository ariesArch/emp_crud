<template>
	<v-stepper-content class="pa-4 section_container" step="2">
		<v-row>
			<v-col md="6">
				<InfoCard :model="model" type="Department" />
			</v-col>
			<v-col md="6">
				<validation-observer ref="observer">
					<v-form ref="form" @submit.prevent="onSaveDepartment">
						<v-card id="no_print">
				...
				if (!this.model.id) {
					try {
						const create_payload =
							this.$model.CREATE_DEPARTMENT_PAYLOAD(this.model);
					...
					try {
						...
						const { message, status, data } =
							await this.$request.putData(
								`${this.$api.DEPARTMENT_URL}/${this.model.id}`,
								update_payload,
								this.jwt
							);
						if (status === 1) {
